...
```
sed -i "s/FROM arch:1607/FROM arch:$(date +%y%m)/" Dockerfile-mylnmp
docker build --force-rm --no-cache --rm -f Dockerfile-mylnmp -t mylnmp:$(date +%y%m%d) .
docker tag mylnmp:$(date +%y%m%d) mylnmp:using

#initialize the MariaDB data directory
docker run --rm -v <host-data-dir>/mysql:/var/lib/mysql:rw mylnmp:using bash -c \
    'mysql_install_db --user=mysql --basedir=/usr --datadir=/var/lib/mysql'
container_id=$(docker run -u mysql -d -v <host-data-dir>/mysql:/var/lib/mysql:rw \
    mylnmp:using bash -c 'mysqld --pid-file=/run/mysqld/mysqld.pid;')
...
docker run --rm -p 80:80 -p 443:443 -v <host-data-dir>/etc:/srv/etc:ro \
    ...
    -v <host-data-dir>/log:/srv/log:rw mylnmp:using
```
